...
// 挂载 OpenList 实例（同 alist_v3 思路，端点 /api/fs/*、/api/me、/api/auth/login）
// 严格对齐 OpenList 源码 drivers/openlist/{driver,util}.go
export class OpenListDriver extends CloudBase {
  ...
  async init(cfg: DriverConfig): Promise<void> {
    ...
    if (!this.address) throw new Error("openlist: 缺少 url");
  }
  ...
  private async ensureToken(): Promise<string> {
    ...
    if (!r.ok) throw new Error(`openlist 登录失败 ${r.status}`);
    const j = (await r.json()) as any;
    if (j.code !== 200) throw new Error(`openlist 登录失败: ${j.message}`);
  ...
  async list(path: string): Promise<FileItem[]> {
    ...
    if (j.code !== 200) throw new Error(`openlist list: ${j.message}`);
  ...
  async get(path: string): Promise<FileItem> {
    ...
    if (j.code !== 200) throw new Error(`openlist get: ${j.message}`);
    const d = j.data;
    return {
      name: basename(path),
      path,
  ...
  async getContent(path: string, range?: string): Promise<Response | string> {
    ...
    if (j.code !== 200) throw new Error(`openlist getContent: ${j.message}`);
  ...
  async createUpload(path: string, _size: number): Promise<UploadSession> {
    return { uploadUrl: `/api/fs/put?path=${encodeURIComponent(path)}`, method: "PUT", headers: { "x-driver": "openlist" } };
  }
  ...
  async putContent(path: string, body: ReadableStream, contentType?: string, size = 0): Promise<void> {
    ...
    if (!r.ok) {
      ...
      throw new Error(`openlist 上传失败 ${r.status}: ${txt}`);
    }
  }
